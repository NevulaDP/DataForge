...
const Mentor: React.FC<Props> = ({ scenario, blocks, messages, setMessages }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await getMentorAdvice(scenario, messages, blocks);
  ...
  return (
    <div className="fixed bottom-12 right-12 z-50">
      {!isOpen ? (
        ...
        <div className="w-[480px] h-[720px] bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-[48px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-12 duration-300">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex space-x-4 max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 shadow-lg ${msg.role === 'user' ? 'bg-blue-600' : 'bg-slate-100 dark:bg-slate-800'}`}>
                    {msg.role === 'user' ? <User className="w-5 h-5 text-white" /> : <Bot className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
                  </div>
                  <div className={`p-5 rounded-[32px] text-sm leading-relaxed font-bold ${
                    msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-tl-none border border-transparent dark:border-slate-700/50'
                  }`}>
                    {msg.content}
                  </div>
                </div>
          ...
          <div className="p-8 border-t-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-100 dark:bg-slate-950 border-2 border-transparent dark:border-slate-800 rounded-[28px] py-5 pl-8 pr-16 text-base text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-800 focus:outline-none focus:border-blue-600 focus:bg-white dark:focus:bg-slate-900 transition-all font-bold"
              />
              <button
                onClick={handleSend}
